...
import { AlertTriangle, CalendarDays, ChevronDown, CloudRain, Compass, ExternalLink, MapPin, Navigation, ShieldCheck, Umbrella, Wind } from "lucide-react";
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-[#f3f7fb] text-[#13233b]">
    ...
    <div className="mx-auto grid w-full max-w-[1680px] gap-5 p-3 sm:p-4 md:p-8 xl:grid-cols-[minmax(520px,620px)_minmax(0,1fr)]">
      <aside className="order-2 min-w-0 space-y-5 xl:order-1">
      ...
      <div className="order-1 min-w-0 space-y-5 xl:order-2">
        ...
        <section className="rounded-[28px] border border-blue-200 bg-white p-4 shadow-sm sm:p-6" aria-labelledby="today-summary"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-black text-blue-600">{day === "today" ? "今日" : "明日"}の結論</p><h2 id="today-summary" className="mt-1 text-2xl font-black">{pref}の外出チェック</h2></div><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-blue-600 text-3xl shadow-md">{selectedForecast?.icon ?? "…"}</span></div>
          <div className="mt-4 rounded-2xl bg-blue-50 p-4"><p className="text-lg font-black text-blue-950">{umbrellaAdvice}</p><p className="mt-1 text-sm font-bold text-blue-700">24時間の予想降水量 {forecast ? `${dayRainTotal}mm` : "--"}</p></div>
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="rounded-2xl bg-slate-50 p-3"><span className="text-sm font-bold text-slate-500">最高 / 最低</span><b className="mt-1 block text-lg"><span className="text-rose-500">{selectedForecast?.high ?? "--"}°</span> / <span className="text-blue-500">{selectedForecast?.low ?? "--"}°</span></b></div><div className="rounded-2xl bg-slate-50 p-3"><span className="text-sm font-bold text-slate-500">雨のピーク</span><b className="mt-1 block text-lg">{maxHourlyRain > 0 && maxRainIndex >= 0 ? `${hourlyRain[maxRainIndex].fullLabel}・${maxHourlyRain}mm/h` : "目立った雨なし"}</b></div><div className="rounded-2xl bg-slate-50 p-3"><span className="text-sm font-bold text-slate-500">風のピーク</span><b className="mt-1 block text-lg">{forecast && maxWindIndex >= 0 ? `${hourlyRain[maxWindIndex].fullLabel}・${dayMaxWind}km/h` : "--"}</b></div><div className={`rounded-2xl border p-3 ${warningTheme}`}><span className="text-sm font-bold opacity-70">警報・注意報</span><b className="mt-1 block text-base">{warningStatus.title}</b></div></div>
          <div className="mt-3 flex flex-wrap gap-2 text-sm font-bold"><span className={`rounded-full px-3 py-2 ${forecastStability === "予報が割れています" ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"}`}>{forecastStability}</span><span className="rounded-full bg-slate-100 px-3 py-2 text-slate-700">{validRows.length}/{providers.length}モデル有効</span><span className="rounded-full bg-slate-100 px-3 py-2 text-slate-700">多数派：{majorityWeather} {agreement}/{validRows.length || providers.length}</span><span className="rounded-full bg-sky-100 px-3 py-2 text-sky-800">降水確率 {rainRange}</span><span className="rounded-full bg-orange-100 px-3 py-2 text-orange-800">気温幅 {temperatureRange}</span></div>
        </section>
        <section className={`rounded-2xl border p-4 ${warningTheme}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-white/80"><AlertTriangle size={22}/></span><div><h2 className="text-lg font-black">{pref}：{warningStatus.title}</h2>{warningStatus.names.length > 0 && <p className="mt-1 text-sm font-bold">{warningStatus.names.slice(0, 4).join("・")}{warningStatus.names.length > 4 ? ` ほか${warningStatus.names.length - 4}件` : ""}</p>}<p className="mt-1 text-sm opacity-80">{warningStatus.headline || "気象庁発表の最新情報です。"}{warningStatus.updated && `（${warningStatus.updated}発表）`}</p></div></div><div className="grid shrink-0 grid-cols-2 gap-2 sm:flex"><a href={warningUrl} target="_blank" rel="noreferrer" className="min-h-11 rounded-xl border border-current bg-white px-4 py-2.5 text-center text-sm font-black">詳細を確認</a><a href={riskMapUrl} target="_blank" rel="noreferrer" className="min-h-11 rounded-xl bg-amber-600 px-4 py-2.5 text-center text-sm font-black text-white">キキクル</a></div></div></section>
        <section className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-[#2872ff] via-[#438cf8] to-[#79b7f6] p-5 text-white shadow-xl shadow-blue-200/70 sm:rounded-[32px] sm:p-6 md:p-8"><div className="absolute -right-10 -top-20 size-64 rounded-full bg-white/10"/><div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-center"><div><div className="flex flex-wrap items-center gap-2 text-base font-bold text-blue-100 sm:text-sm"><Navigation size={18}/>{pref} · {day === "today" ? "今日" : "明日"} <span className="rounded-full bg-white/20 px-2 py-1 text-xs">実予報モデル</span></div><h2 className="mt-4 text-[28px] font-black leading-tight sm:text-3xl md:text-4xl">{forecastError || (forecast ? `${selectedForecast?.weather ?? "取得中"}の予報` : "予報を取得しています…")}</h2><p className="mt-3 text-base leading-7 text-blue-50">JMA・NOAA GFS・ECMWF IFS・DWD ICON Globalの最新値を比較しています。</p></div><div className="flex items-center justify-center gap-4 md:justify-start md:pr-6"><div className="grid size-24 shrink-0 place-items-center rounded-full bg-white/90 shadow-inner"><span className="text-6xl" role="img" aria-label={selectedForecast?.weather ?? "取得中"}>{selectedForecast?.icon ?? "…"}</span></div><div><div className="text-6xl font-light">{selectedForecast?.high ?? "--"}°</div><div className="mt-1 text-base font-bold">最低 {selectedForecast?.low ?? "--"}°</div></div></div></div>
          <div className="relative mt-7 grid grid-cols-3 gap-3">{[{i:<Umbrella key="i"/>,l:"降水確率",v:selectedForecast?.rain == null ? "--" : `${selectedForecast.rain}%`},{i:<ShieldCheck key="i"/>,l:"天気の一致",v:`${agreement} / ${providers.length}`},{i:<Wind key="i"/>,l:"最大風速平均",v:averageWind == null ? "--" : `${averageWind} km/h`}].map(x=><div key={x.l} className="rounded-2xl bg-white/15 p-3 backdrop-blur"><div className="flex items-center gap-2 text-sm text-blue-50">{x.i}{x.l}</div><div className="mt-1 text-xl font-black">{x.v}</div></div>)}</div>
        </section>
