  ...
  return <main className="min-h-screen bg-[#f3f7fb] text-[#13233b]">
    ...
    <div className="mx-auto grid w-full max-w-[1680px] gap-5 p-3 sm:p-4 md:p-8 xl:grid-cols-[minmax(520px,620px)_minmax(0,1fr)]">
      ...
      <div className="order-1 min-w-0 space-y-5 xl:order-2">
        ...
        <section className="rounded-[28px] border border-blue-200 bg-white p-4 shadow-sm sm:p-6" aria-labelledby="today-summary"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-sm font-black text-blue-600">実予報モデル</p><h2 id="today-summary" className="mt-1 text-2xl font-black">{pref} · {day === "today" ? "今日" : "明日"}</h2><p className="mt-1 text-lg font-black text-slate-700">{forecastError || (forecast ? `${selectedForecast?.weather ?? "取得中"}の予報` : "予報を取得しています…")}</p><p className="mt-1 text-sm text-slate-500">JMA・GFS・ECMWF・ICONの最新値を比較</p></div><div className="flex items-center gap-3"><span className="grid size-16 shrink-0 place-items-center rounded-2xl bg-blue-600 text-4xl shadow-md" role="img" aria-label={selectedForecast?.weather ?? "取得中"}>{selectedForecast?.icon ?? "…"}</span><div><b className="block text-4xl font-black text-slate-800">{selectedForecast?.high ?? "--"}°</b><span className="text-base font-bold text-blue-600">最低 {selectedForecast?.low ?? "--"}°</span></div></div></div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="rounded-2xl bg-slate-50 p-3"><span className="text-sm font-bold text-slate-500">降水確率平均</span><b className="mt-1 block text-xl text-blue-700">{selectedForecast?.rain == null ? "--" : `${selectedForecast.rain}%`}</b></div><div className="rounded-2xl bg-slate-50 p-3"><span className="text-sm font-bold text-slate-500">雨のピーク</span><b className="mt-1 block text-lg">{maxHourlyRain > 0 && maxRainIndex >= 0 ? `${hourlyRain[maxRainIndex].fullLabel}・${maxHourlyRain}mm/h` : "目立った雨なし"}</b></div><div className="rounded-2xl bg-slate-50 p-3"><span className="text-sm font-bold text-slate-500">最大風速平均</span><b className="mt-1 block text-xl">{averageWind == null ? "--" : `${averageWind}km/h`}</b><span className="mt-1 block text-xs font-bold text-slate-400">ピーク {forecast && maxWindIndex >= 0 ? hourlyRain[maxWindIndex].fullLabel : "--"}</span></div><div className={`rounded-2xl border p-3 ${warningTheme}`}><span className="text-sm font-bold opacity-70">警報・注意報</span><b className="mt-1 block text-base">{warningStatus.title}</b></div></div>
