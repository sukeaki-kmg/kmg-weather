...
export default function Home() {
  ...
  const umbrellaAdvice = !forecast ? "降水データを取得中…" : umbrellaHours.length === 0 ? "今後24時間は傘なしで過ごせそうです" : `${umbrellaHours[0].fullLabel}〜${umbrellaHours.at(-1)?.fullLabel}は傘推奨`;
    ...
    <div className="mx-auto grid w-full max-w-[1680px] gap-5 p-3 sm:p-4 md:p-8 xl:grid-cols-[minmax(520px,620px)_minmax(0,1fr)]">
      ...
      <div className="min-w-0 space-y-5">
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm md:p-6"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-blue-600">NEXT 24 HOURS</p><h2 className="text-xl font-black">更新時刻から先の24時間</h2><p className="mt-1 text-xs text-slate-400">{pref}・過ぎた時間を除いた実予報モデルの平均</p></div><CloudRain className="mt-1 text-blue-500" size={25}/></div>
          ...
          <div className="mt-4 grid grid-cols-[repeat(24,minmax(0,1fr))] overflow-hidden rounded-xl border border-slate-200 text-center text-xs">{hourlyRain.map((h,index)=><div key={`${h.fullLabel}-${index}`} className="border-r border-slate-100 px-1 py-2 last:border-0"><span className="block h-4 text-[10px] font-bold text-blue-500">{h.dateLabel}</span><span className="block font-bold text-slate-600">{h.time}</span><span className="mt-1 block text-[10px] text-slate-400">降水確率</span><b className="block text-blue-700">{h.chance}%</b><span className="mt-1 block text-[10px] text-slate-400">降水量</span><b className="block text-sky-700">{h.precipitation}mm</b><span className="mt-1 block text-[10px] text-slate-400">風速</span><b className="block text-slate-700">{h.wind}km/h</b></div>)}</div></div></div>
        </section>
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm sm:p-5 md:p-6 xl:hidden"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold text-blue-600 sm:text-xs">FORECAST SOURCES · {day === "today" ? "TODAY" : "TOMORROW"}</p><h2 className="text-xl font-black">4つの予報を比較</h2></div><p className="text-xs font-medium text-slate-500" aria-live="polite">最終更新 {lastUpdated || "--:--"} · 1時間ごとに自動更新</p></div>
