      ...
      <aside className="space-y-5">
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-2 flex items-center justify-between"><div><p className="text-xs font-bold text-blue-600">全国の今日の天気</p><h2 className="font-black">主要12都市を一目で確認</h2></div><MapPin className="text-blue-500"/></div>
          <div className="relative mx-auto aspect-square w-full max-w-[520px] overflow-hidden rounded-2xl bg-gradient-to-b from-sky-50 to-blue-50/40">
            ...
            {cities.map(c=><button key={c.name} onClick={()=>{setRegion(c.region);setPref(c.pref)}} style={{left:`${c.left}%`,top:`${c.top}%`}} className="absolute z-20 hidden min-w-[72px] -translate-x-1/2 items-center gap-1 rounded-xl border border-white bg-white/95 px-2 py-1 text-left shadow-lg transition hover:z-30 hover:scale-110 md:flex"><span className="text-xl">{c.icon}</span><span><b className="block text-[10px] leading-none">{c.name}</b><b className="text-xs">{c.temp}</b></span></button>)}
            <span className="absolute bottom-2 right-3 text-[9px] font-medium text-slate-400">地図: Geolonia / GFDL</span>
          </div>
          <div className="mt-3 grid grid-cols-3 gap-2 md:hidden">{cities.map(c=><button key={c.name} onClick={()=>{setRegion(c.region);setPref(c.pref)}} className="flex min-h-14 items-center justify-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-1 py-2 text-left active:bg-blue-50"><span className="text-xl">{c.icon}</span><span><b className="block text-[10px] leading-none">{c.name}</b><b className="text-xs">{c.temp}</b></span></button>)}</div>
