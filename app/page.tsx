      ...
      <aside className="space-y-5">
        ...
        <section className="rounded-[24px] border border-slate-200 bg-white p-3 shadow-sm sm:rounded-[28px] sm:p-5"><div className="mb-2 flex items-center justify-between"><div><p className="text-xs font-bold text-blue-600">全国の{day === "today" ? "今日" : "明日"}の天気</p><h2 className="font-black">主要12都市を一目で確認</h2></div><MapPin className="text-blue-500"/></div>
          <div className="relative mx-auto aspect-square w-full max-w-[660px] overflow-hidden rounded-2xl bg-gradient-to-b from-sky-50 to-blue-50/40">
            ...
            {mapCities.map(c=><button key={c.name} onClick={()=>{setRegion(c.region);setPref(c.pref)}} style={{left:`${c.left}%`,top:`${c.top}%`}} className="absolute z-20 flex min-w-[78px] -translate-x-1/2 items-center justify-center gap-1.5 rounded-xl border-2 border-white bg-white/95 px-2.5 py-2 text-left shadow-xl transition hover:z-30 hover:scale-110 md:min-w-[112px] md:gap-2 md:px-3.5 md:py-2.5"><span className="text-[28px] leading-none md:text-[42px]">{c.icon}</span><span><b className="block text-[11px] leading-none md:text-sm">{c.name}</b><b className="text-[15px] leading-tight md:text-xl">{c.temp}</b></span></button>)}
